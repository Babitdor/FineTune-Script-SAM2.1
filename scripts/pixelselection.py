...
def select_pixels(segmentation_mask, class_id, num_points, selection_method="laplace"):
    ...
    if len(xs) < num_points:
        raise ValueError(
            f"Not enough pixels for class {class_id}: found {len(xs)} but need {num_points}."
        )
    ...
    if selection_method.lower() == "fibonacci":
        ...
        raise ValueError(
            "Unknown selection method. Choose 'fibonacci', 'laplace', 'shuffle', or 'gaussian'."
        )
...
def shuffle_selection(mask: np.ndarray, class_id: int, num_pixels: int) -> np.ndarray:
    ...
    if total_pixels <= num_pixels:
        ...
        selected_coords = coords[::distance][
            :num_pixels
        ]  # Select pixels with step size
...
def select_centroid(segmentation_mask, class_id, num_points):
    ...
    for instance_id in range(1, num_features + 1):
        com = center_of_mass(
            binary_mask, labeled_mask, instance_id
        )  # (row, col) as float
    ...
    extra_coords = np.array(
        [tuple(coord) for coord in all_coords if tuple(coord) not in centroid_set]
    )
    ...
    if len(extra_coords) < additional_needed:
        ...
        indices_extra = np.random.choice(
            len(extra_coords), additional_needed, replace=False
        )
        additional_points = extra_coords[indices_extra]

    # Convert additional points from (row, col) to (x, y) i.e. (col, row).
    additional_points_xy = np.column_stack(
        [additional_points[:, 1], additional_points[:, 0]]
    )
    ...
    *,
    ...
    for coords_rc, k in zip(coords_by_inst, alloc):
        if k == 1 or coords_rc.shape[0] <= 30:  # tiny blob → centroid
            ...
            seed_rc = np.unravel_index(
                np.argmax(distance_transform_edt(mask_inst)), mask_inst.shape
            )

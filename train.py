...
def train(
    ...
    for epoch in tqdm(range(1, (epochs + 1)), desc="Training Epochs Completed"):
        ...
        # Unfreeze last layers of image encoder
        ...
        for train_step, data_row in tqdm(
            ...
            with torch.amp.autocast(device_type="cuda"):
                ...
                prd_mask = prd_masks[:, 0]  # No sigmoid here
        ...
        writer.add_scalar("Training/BCE", mean_bce, epoch)
        ...
        torch.save(
            ...
            f"checkpoints/model_checkpoints/sam2_test_epoch_{epoch+1}.pt",
        )
...
def validation(predictor, scheduler, criterion, val_data, writer, epoch):
    ...
    with torch.no_grad():
        ...
        writer.add_scalar("Validation/BCE", avg_bce, epoch)
...
if __name__ == "__main__":
    ...
    PRETRAINED_MODEL = "sam2.1_hiera_base_plus.pt"
    ...
    # gt_mask[:, 1:3, 1:3] = 1
    # pred_mask = torch.full((1, 4, 4), -10.0)  # logits for 0
    # pred_mask[:, 1:3, 1:3] = 10.0

    # # Dummy pred_scores (if needed by your loss)
    # pred_scores = torch.tensor([[0.5]])

    # # Calculate loss
    # loss, bce, dice, score, iou = criterion(pred_mask, gt_mask, pred_scores)
    ...
    torch.save(predictor.model.state_dict(), "model/sam2_test_final_model.pt")

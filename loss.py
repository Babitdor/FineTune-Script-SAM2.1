...
class SegmentationLoss(nn.Module):
    ...
    def score_loss(self, pred_score, actual_iou):
        ...
        elif self.score_loss_type == "kl":
            return F.kl_div(pred_score.log(), actual_iou, reduction="batchmean")
        else:  # combo loss
    ...
    def forward(self, pred_mask, gt_mask, pred_scores):
        ...
        pred_scores = pred_scores.to(pred_mask.device)
        focal = self.sigmoid_focal_loss(pred_mask, gt_mask)
        dice = self.generalized_dice_loss(torch.sigmoid(pred_mask), gt_mask)
        iou = self.iou_score(pred_mask.sigmoid(), gt_mask, epsilon=1e-6).detach()
        score = self.score_loss(torch.sigmoid(pred_scores), iou)
        total = (
            focal * self.weights[0] + dice * self.weights[1] + score * self.weights[2]
        )
        return total, focal, dice, iou, score
